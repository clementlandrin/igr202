...
void keyCallback (GLFWwindow * windowPtr, int key, int scancode, int action, int mods) {
	...
	} else if (action == GLFW_PRESS && key == GLFW_KEY_1 && shaderMode != 0){
		switchShaderMode(1);
	} else if (action == GLFW_PRESS && key == GLFW_KEY_2 && shaderMode != 0){
		switchShaderMode(2);
	} else if (action == GLFW_PRESS && key == GLFW_KEY_3 && shaderMode != 0){
		switchShaderMode(3);
	} else if (action == GLFW_PRESS && key == GLFW_KEY_4 && shaderMode != 0){
		switchShaderMode(4);
...
int main (int argc, char ** argv) {
	if (argc > 2)
		usage (argv[0]);
	init (argc == 1 ? DEFAULT_MESH_FILENAME : argv[1]); // Your initialization code (user interface, OpenGL states, scene with geometry, materialPtr, lights, etc)
